...
public class RedBlackPanel extends JPanel implements KeyListener{

    private BinaryTree tree = new RedBlackTree();
    private String readValues = "";
    private Boolean displayedWarning = false;
    private int drawStyle = 0; 
    private int numStyle = 0;

    public RedBlackPanel(){
        addKeyListener(this);
    }
    ...
    public void drawNode(Graphics g, BinaryNode n, int x, int y, int level){
        ...
        centerCircle(g, x, y, isBlack);
        //g.setColor(Color.BLACK);
        centerNumber(g, n, x, y);
    }

    public void centerCircle(Graphics g, int x, int y, Boolean isBlack){
        if(isBlack){
